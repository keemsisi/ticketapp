...
public class NotificationService extends BaseRepoService<Notification> implements INotification {
    ...
    public Page<Notification> getAllByActionNameStatusAndModuleIdPaged(String actionName, ApprovalStatus status, UUID moduleId, Date startDate, Date endDate, Pageable pageable) {
        ...
        if (ObjectUtils.isNotEmpty(actionName) && ObjectUtils.isNotEmpty(status) && ObjectUtils.isNotEmpty(startDate) && ObjectUtils.isNotEmpty(endDate)) {
            ...
            if (ObjectUtils.isEmpty(moduleId))
                return notificationRepository.getAllStartDateAndEndDatePaged(startDate, endDate, tenantId, NotificationType.SUBSCRIPTION.name(), pageable);
            ...
            if (ObjectUtils.isEmpty(moduleId))
                return notificationRepository.getAllByStatus(status.name(), tenantId, NotificationType.SUBSCRIPTION.name(), pageable);
    ...
    public List<Notification> getAllByActionNameStatusAndModuleIdUnPaged(String actionName, ApprovalStatus status, Date startDate, Date endDate, UUID moduleId) {
        ...
        if (ObjectUtils.isNotEmpty(actionName) && ObjectUtils.isNotEmpty(status) && ObjectUtils.isNotEmpty(startDate) && ObjectUtils.isNotEmpty(endDate)) {
            ...
            if (ObjectUtils.isEmpty(moduleId))
                return notificationRepository.getAllStartDateAndEndDateUnPaged(startDate, endDate, tenantId, NotificationType.SUBSCRIPTION.name());
            ...
            if (ObjectUtils.isEmpty(moduleId))
                return notificationRepository.getAllByStatus(status.name(), tenantId, NotificationType.SUBSCRIPTION.name());
    ...
    private List<NotificationSubscriber> getSubscribersCurrentSessionIds(String actionName, UUID tenantId) {
        return iNotificationDao.getAllMatchingSubscriberCurrentSessionIds(actionName, tenantId);
    }
...
//                        reliefRequestService.sendMailToReliefOfficer();
                ...
                } else if (notification.getNotificationType() == NotificationType.SUBSCRIPTION) {
    ...
    public UserNotificationApprovalStatusStats getUserNotificationApprovalStatusStats() {
        LoggedInUserDto loggedInUserDto = jwtTokenUtil.getUser();
        Map<String, Object> results = iNotificationDao.getUserNotificationStats(loggedInUserDto.getScope(), loggedInUserDto.getRoles(), loggedInUserDto.getUserId(), loggedInUserDto.getTenantId());
    ...
    public List<UserNotificationGroupByDateCreatedWrapper> fetchUserNotificationsReceivedByDateRange(Date startDate, Date endDate) {
        return iNotificationDao.getUserNotificationsReceivedByDateRange(jwtTokenUtil.getUser().getScope(), startDate, endDate, jwtTokenUtil.getUser().getTenantId());
    }

    @Override
    public List<UserNotificationModuleStatsWrapper> getUserModuleStats() {
        return iNotificationDao.getUserModuleStats(jwtTokenUtil.getUser().getScope(), jwtTokenUtil.getUser().getTenantId());
    }

    @Override
    public List<UserUnreadNotificationStatsByModuleId> getUserUnreadNotificationsStatsByModuleId(UUID moduleId) {
        return iNotificationDao.getUserNotificationStatsByModuleId(jwtTokenUtil.getUser().getUserId(), jwtTokenUtil.getUser().getScope(), jwtTokenUtil.getUser().getTenantId(), moduleId);
    }

    @Override
    public List<Notification> getAllNotifications(@NotNull PageRequestParam prp) throws ParseException {
        return iNotificationDao.getAllNotifications(jwtTokenUtil.getUser().getUserId(), jwtTokenUtil.getUser().getRoles(),
