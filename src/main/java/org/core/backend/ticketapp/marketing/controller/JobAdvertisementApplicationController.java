...
import org.core.backend.ticketapp.marketing.dto.job_advertisement.application.CreateJobAdvertisementApplicationRequest;
...
@Slf4j
@RestController
@RequestMapping("/api/v1/jobs/advertisements/application")
@AllArgsConstructor
public class JobAdvertisementApplicationController {
    private JobAdvertisementApplicationService service;

    @RequestMapping(method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<GenericApiResponse<JobAdvertisementApplication>> create(@Validated @RequestBody CreateJobAdvertisementApplicationRequest request) throws Exception {
    ...
    public ResponseEntity<GenericApiResponse<JobAdvertisementApplication>> getById(final @PathVariable UUID id) {
    ...
    public ResponseEntity<GenericApiResponse<JobAdvertisementApplication>> deleteById(final @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok().body(new GenericApiResponse<>("00", "Successfully deleted resource!", null));
    }

    @RequestMapping(method = RequestMethod.PATCH, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<GenericApiResponse<JobAdvertisementApplication>> update(final @RequestBody UpdateJobAdvertisementApplicationRequest request) {
