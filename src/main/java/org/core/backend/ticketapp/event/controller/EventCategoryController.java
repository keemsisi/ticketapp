...
public class EventCategoryController implements ICrudController {
    ...
    public ResponseEntity<GenericResponse<EventCategory>> create(@Valid @RequestBody EventCategoryCreateRequestDTO request) {
        ...
        return new ResponseEntity<>(new GenericResponse<>("00", "Created successfully", category), HttpStatus.CREATED);
    }

    @RequestMapping(method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, value = "/{id}")
    public ResponseEntity<GenericResponse<EventCategory>> getById(UUID id) {
        final var category = categoryService.getById(id);
        return new ResponseEntity<>(new GenericResponse<>("00", "Created successfully", category), HttpStatus.OK);
    }

    @RequestMapping(method = RequestMethod.PATCH, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<GenericResponse<EventCategory>> update(@RequestBody EventCategoryUpdateRequestDTO request) {
        final var category = categoryService.update(request);
        return new ResponseEntity<>(new GenericResponse<>("00", "Updated successfully", category),
                HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> getAll() {
        ...
        return new ResponseEntity<>(new GenericResponse<>("00", "All event categories", categories), HttpStatus.OK);
    }

    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<GenericResponse<EventCategory>> delete(@PathVariable UUID id) {
