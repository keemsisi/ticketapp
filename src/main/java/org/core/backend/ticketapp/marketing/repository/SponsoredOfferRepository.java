...
public interface SponsoredOfferRepository extends JpaRepository<SponsoredOffer, UUID> {
    @Query(value = "SELECT * FROM sponsored_offer WHERE user_id = ?1 AND deleted = false AND end_date >= now() ", nativeQuery = true)
    Page<SponsoredOffer> findAll(final @NotNull UUID userId, final @NotNull Pageable pageable);

    @Query(value = "SELECT * FROM sponsored_offer WHERE deleted = false AND end_date >= now() ", nativeQuery = true)
    @org.jetbrains.annotations.NotNull
    Page<SponsoredOffer> findAll(final @NotNull @org.jetbrains.annotations.NotNull Pageable pageable);
