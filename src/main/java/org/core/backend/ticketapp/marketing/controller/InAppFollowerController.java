...
public class InAppFollowerController {
    ...
    public ResponseEntity<GenericResponse<InAppFollower>> create(@Validated @RequestBody CreateInAppFollowerRequest request) throws Exception {
    ...
    public ResponseEntity<GenericResponse<InAppFollower>> getById(final @PathVariable UUID id) {
    ...
    public ResponseEntity<GenericResponse<InAppFollower>> deleteById(final @PathVariable UUID id) {
        service.delete(id);
        return ResponseEntity.ok().body(new GenericResponse<>("00", "Successfully deleted resource!", null));
    }

    @RequestMapping(method = RequestMethod.PATCH, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<GenericResponse<InAppFollower>> update(final @RequestBody UpdateInAppFollowerRequest request) {
