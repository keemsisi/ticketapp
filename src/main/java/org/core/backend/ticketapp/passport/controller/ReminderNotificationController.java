...
public class ReminderNotificationController {
    ...
    public ResponseEntity<GenericApiResponse<?>> createNewReminderNotification(@Validated @RequestBody ReminderNotificationDto reminderNotificationDto) throws JsonProcessingException {
        return ResponseEntity.ok().body(new GenericApiResponse<>("00", "reminder created", iReminderNotification.createNewReminder(reminderNotificationDto)));
    }
    ...
    public ResponseEntity<?> getAll(@RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date expiryDate, @RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date dateCreated, @RequestParam(required = false) Integer pageNumber, @RequestParam(required = false) Integer pageSize, @RequestParam(required = false) Sort.Direction order, @RequestParam(required = false) String[] sortBy) {
        return new ResponseEntity<>(new GenericApiResponse<>("00", "Reminder notifications fetched successfully", iReminderNotification.getAll(expiryDate, dateCreated, ResponsePageRequest.createPageRequest(pageNumber, pageSize, order, sortBy, true, "date_created"))), HttpStatus.OK);
    }

    @RequestMapping(method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE, value = "/read-receipt")
    public ResponseEntity<GenericApiResponse<?>> readNotification(@Valid @RequestBody NotificationReadDTO notificationReadDTO) throws JsonProcessingException {
    ...
    public ResponseEntity<GenericApiResponse<?>> deleteReminderNotification(@PathVariable(value = "id") UUID id) throws JsonProcessingException {
        iReminderNotification.delete(id);
        return ResponseEntity.ok().body(new GenericApiResponse<>("00", "deleted reminder notification with provided id", id));
    }

    @RequestMapping(method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, value = "/view/{id}")
    public ResponseEntity<GenericApiResponse<ReminderNotification>> getReminderNotificationById(@PathVariable(value = "id") UUID id) {
        return ResponseEntity.ok().body(new GenericApiResponse<>("00", "reminder notification found", iReminderNotification.getReminderNotificationById(id)));
    }
    ...
    public ResponseEntity<?> updateReminderNotification(@Validated @RequestBody ReminderUpdateDto reminderUpdateDto) throws JsonProcessingException {
        ...
        return ResponseEntity.ok().body(new GenericApiResponse<>("00", "update was successful", null));
    }
}